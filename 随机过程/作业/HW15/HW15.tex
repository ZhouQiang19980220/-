...
\begin{solution}
    \begin{itemize}
        ...
        \begin{align*}
            ...
            \\
            &=
            \frac{1}{4}t^2s^2\sigma_2^2
            +
            ts\sigma_1 ^ 2
            +
            \frac{1}{2}ts(t+s)\rho \sigma_1 \sigma_2
        \end{align*}
        \item [(2)]
        由第(1)问可知，
        \begin{align*}
            R_x(s,t)=            \sigma_1 ^2 + st \sigma_2^2 + (t+s)\rho \sigma_1  \sigma_2
        \end{align*}
        则$X(t)$ 在 $t>0$ 上均方连续、均方可导。
        \item [(3)]
        首先需要证明$Y(t),Z(t)$均方可导，计算二者的相关函数如下。
        \begin{align*}
            R_Y(s,t)=            \frac{1}{4}t^2s^2\sigma_2^2
            +
            ts\sigma_1 ^ 2
            +
            \frac{1}{2}ts(t+s)\rho \sigma_1 \sigma_2
    ...
    \end{align*}
    由于$E\{X^2(u)X^2(v)\}$是u,v的多项式函数，则$R_z(s,t)$的二阶偏导数一定存在，即$Z(t)$可导，其导数随机过程为$X^2(t)$
    \end{itemize}
\end{solution}
